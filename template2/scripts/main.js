...
function init() {
    ...
    loginButton.onclick = function() {
        ...
        loginEmail += "@goa.bits-pilani.ac.in";


        firebase.auth().signInWithEmailAndPassword(loginEmail, loginPassword).then(function(snapshot) {
                //logged in
...
function eventsButtonClick() {
    ...
    database.ref('Event').on('value', function(snapshot) {
        ...
        var str = '<thead> EVENTS LIST </thead><th>Event Name</th> <th> Venue </th> <th> Entry Time </th> <th>Exit Time</th>';
        for (var key in data) {
            if (data.hasOwnProperty(key)) {
                var buttonId = key + 'button';
                //converting firebase data to html
                str += "<tr> <td>" + data[key].name + "</td><td>" + data[key].place + "</td><td>" + data[key].start + "</td><td>" + data[key].end + '</td><td><button class="btn" id="' + buttonId + '">X</button></td></tr>';
            }
        }
            ...
            database.ref('Users').once('value', function(snapshot) {
                ...
                function eventInventoryListButtonClick() {
                    ...
                    for (var key in eventList) {
                        ...
                        str += '<button type="button" class="btn" id="' + buttonId + '">' +
                        ...
                        button.onclick = function(eventObj) {
                            for (var key in eventList) {
                                ...
                                if (eventObj.target.id == buttonId) {
                                    ...
                                    eventObj.target.style.color = '#FFFFFF';
                                } else {
                                    var button = document.getElementById(buttonId);
                                    button.style.fontWeight = 'normal';
                                    button.style.color = '#9FB1B2';
                                }
                            }
                            displayInventoryList(eventObj.target);
                        };
                ...
                function displayInventoryList(eventObjTarget) {
                    ...
                    var str = '<thead> INVENTORY LIST </thead><th>Item Number</th> <th>Event</th> <th>Issued To</th> <th>Issue Time</th> <th>Return Time</th>';
                        ...
                        if (inventoryList.hasOwnProperty(key)) {
                            ...
                            if (displayType == eventKey) {
                                ...
                                str += "<tr> <td>" + inventoryList[key].item + "</td><td>" + eventList[eventKey].name + "," + eventList[eventKey].place +
                                    ...
                                    '</td><td><button class="btn" id="' + buttonId + '">' + buttonText + '</button></td></tr>';
                    ...
                    inventoryListTable.style.display = "block";
...
function membersButtonClick() {
    ...
    database.ref('Users').once('value', function(snapshot) {
        ...
        var str = '<thead> MEMBERS </thead><th>Name</th> <th>Role</th> <th>Contact</th> <th>BITS ID</th>';
